...
AddEventHandler('gcPhone:buyMarket', function(itemName, amount, price)
	...
	if xPlayer.getBank() >= price then
		...
		if (sourceItem.count + amount) > sourceItem.limit then
			TriggerClientEvent('esx:showNotification', _source, "You don't have enough inventory!")
		else
			...
			TriggerClientEvent('esx:showNotification', _source, "Account deducted from: " .. price .. " $ ")
		end
	else
		local missingMoney = price - xPlayer.getBank()
		TriggerClientEvent('esx:showNotification', _source, "You don't have enough money, you missing " .. missingMoney .. "$~s~!")
	end
end)
